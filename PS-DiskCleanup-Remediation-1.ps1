...
.NOTES
    ...
    1.1.0 - (2026-04-23) Added:
            - Clear user profile Temp folder contents immediately
            - Clear SCCM client cache content older than 7 days (COM API with folder fallback)
#>
Begin {
...
Process {
    ...
    # ==========================
    # NEW: User profile Temp cleanup + SCCM cache cleanup helper functions
    # ==========================
    function Get-LocalUserProfiles {
        # Returns profile paths for all non-system, non-special profiles
        $SystemProfiles = "S-1-5-18", "S-1-5-19", "S-1-5-20"
        ...
        try {
            Write-LogEntry -Value "Reading user profiles from: $ProfileListKey" -Severity 1
            ...
            foreach ($sidKey in (Get-ChildItem -Path $ProfileListKey -ErrorAction Stop)) {
                ...
                # Skip if profile folder doesn't exist
                if (-not (Test-Path -Path $p.ProfileImagePath)) { continue }

                # Skip common special profiles
            ...
            Write-LogEntry -Value "Failed to enumerate user profiles. Error: $($_.Exception.Message)" -Severity 3
        }
    ...
    function Clear-UserTempFolders {
        ...
        Write-LogEntry -Value "Initiating cleanup of user Temp folders (MinAgeDays=$MinAgeDays)" -Severity 1
        $profiles = Get-LocalUserProfiles

        if ($null -eq $profiles -or $profiles.Count -eq 0) {
            Write-LogEntry -Value "No user profiles found for Temp cleanup" -Severity 2
            return
        }
        ...
        foreach ($userprofile in $profiles) {
            ...
            if (-not (Test-Path -Path $tempPath)) {
                Write-LogEntry -Value "Temp path not found, skipping: $tempPath" -Severity 2
                continue
            }

            Write-LogEntry -Value "Cleaning Temp folder: $tempPath" -Severity 1
            ...
            foreach ($item in $items) {
                try {
                    ...
                    # Expect many failures due to locks
                    Write-LogEntry -Value "Temp cleanup failed for '$($item.FullName)'. Error: $($_.Exception.Message)" -Severity 2
                }
            }
        }

        Write-LogEntry -Value "Cleanup of user Temp folders completed" -Severity 1
    }
    ...
    function Clear-SCCMClientCache {
        ...
        Write-LogEntry -Value "Initiating cleanup of SCCM client cache (MinAgeDays=$MinAgeDays)" -Severity 1
            ...
            foreach ($e in $elements) {
                try {
                    ...
                    if ($MinAgeDays -gt 0) {
                        ...
                        # Try to respect LastReferenceTime if present
                        try {
                        ...
                        if ($null -ne $refTime) {
                            ...
                            # Fallback: check element folder LastWriteTime if location exists
        ...
        if (-not $usedComApi) {
            ...
            foreach ($item in $items) {
                try {
                    if ($MinAgeDays -gt 0 -and $item.LastWriteTime -gt $cutoff) {
    ...
    # Configuration (NEW)
    # ==========================
    $UserTempMinAgeDays = 0          # Clean user temp immediately (best effort)
    ...
    try {
        ...
        foreach ($SageRunSetting in $SageRunSettings) {
            try {
                ...
                New-ItemProperty -Path "HKLM:\SOFTWARE\Microsoft\Windows\CurrentVersion\Explorer\VolumeCaches\$($SageRunSetting)" -Name "StateFlags0001" -Value 2 -PropertyType DWord -ErrorAction "Stop"
                Write-LogEntry -Value "Enabled SageRun setting '$SageRunSetting' (StateFlags0001=2)" -Severity 1
            }
            catch [System.Exception] {
    ...
    # ==========================
    # NEW: Clean user Temp folders immediately
    # ==========================
    try {
        Clear-UserTempFolders -MinAgeDays $UserTempMinAgeDays
    }
    catch [System.Exception] {
        Write-LogEntry -Value "Failed to process user Temp folders. Error message: $($_.Exception.Message)" -Severity 3
        $ExitCode = 1
    }

    # ==========================
    # NEW: Clean SCCM cache older than 7 days
    # ==========================
